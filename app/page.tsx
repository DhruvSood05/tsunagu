...
function Nav({ onSignIn }: { onSignIn: () => void }) {
  ...
  const socials = [
    ...
    },
    {
      label: "Instagram",
      handle: "@dhruvvsood_",
      href: "https://www.instagram.com/dhruvvsood_",
      icon: (
        <svg viewBox="0 0 24 24" className="size-3.5 shrink-0" fill="currentColor" aria-hidden>
    ...
    },
    {
      label: "LinkedIn",
      handle: "Dhruv Sood",
      href: "https://www.linkedin.com/in/dhruv-sood-9871b9272/",
      icon: (
        <svg viewBox="0 0 24 24" className="size-3.5 shrink-0" fill="currentColor" aria-hidden>
        ...
        <div className="hidden sm:flex items-center gap-7 absolute left-1/2 -translate-x-1/2">
          ...
          <div ref={updatesRef} className="relative">
            <button
              ...
              className="flex items-center gap-1 text-[13px] font-medium text-muted-foreground hover:text-foreground transition-colors cursor-pointer bg-transparent"
            >
              Updates
              <svg
                viewBox="0 0 24 24"
                className={`size-3 transition-transform duration-200 ${updatesOpen ? "rotate-180" : ""}`}
                fill="none"
                stroke="currentColor"
                strokeWidth={2.5}
                aria-hidden
            ...
            {updatesOpen && (
              <div className="absolute top-full left-1/2 -translate-x-1/2 mt-3 w-52 bg-card border border-border rounded-2xl shadow-[0_16px_48px_-12px_rgba(0,0,0,0.22)] dark:shadow-[0_16px_48px_-12px_rgba(0,0,0,0.55)] overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                <div className="px-3 pt-3 pb-1">
                  <p className="text-[9px] font-bold uppercase tracking-[0.12em] text-muted-foreground/50 px-1 mb-2">Follow along</p>
                  {socials.map(({ label, handle, href, icon }) => (
                    <a
                      ...
                      className="flex items-center gap-3 px-2 py-2.5 rounded-xl text-muted-foreground hover:text-foreground hover:bg-secondary/70 transition-all group"
                    >
                      <span className="size-7 rounded-lg bg-secondary border border-border/60 flex items-center justify-center shrink-0 group-hover:border-border transition-colors">
                        {icon}
                      </span>
                      <div className="min-w-0">
                        <p className="text-[12px] font-semibold text-foreground leading-tight">{label}</p>
                        <p className="text-[10.5px] text-muted-foreground/60 truncate">{handle}</p>
                      </div>
                    </a>
                  ))}
                </div>
  ...
  const plans = [
    {
      ...
      features: ["Gmail + Calendar connected", "0 AI credits", "Email drafting + scheduling", "Command palette (⌘K)", "Keyboard shortcuts"],
    ...
    },
    {
      ...
      features: ["Everything in Free", "50 AI credits per day", "Priority inbox analysis", "Real-time webhook sync", "Meeting briefs"],
      cta: "Get Your AI Workspace",
      highlight: true,
    },
    {
      name: "Max",
      price: "₹999",
      period: "per month",
      desc: "For power users who need no limits.",
      features: ["Everything in Pro", "Unlimited AI credits", "Shared AI conversation history", "Advanced calendar view", "Priority support"],
    ...
    <footer className="bg-footer text-muted-foreground border-t border-border px-6 pt-16 pb-10">
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-10 mb-16">
          ...
          {/* Socials column */}
          <div>
            <p className="text-[10px] font-bold uppercase tracking-[0.1em] text-[#8f8f8f] mb-4 font-heading">Socials</p>
            <ul className="space-y-3">
              {socials.map(({ label, href, icon }) => (
                <li key={label}>
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-2.5 text-sm text-[#a1a1aa] hover:text-[#f8f7f4] transition-colors group"
                  >
                    <span className="size-6 rounded-lg bg-white/[0.06] border border-white/[0.08] flex items-center justify-center shrink-0 group-hover:bg-white/[0.10] transition-colors">
                      {icon}
                    </span>
                    {label}
                  </a>
